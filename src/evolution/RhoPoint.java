...
////////////////////////////
    ...
    public void geog2Grid() {
        ...
        double[] pGrid = Dataset_EVOL.geo2Grid(lon, lat);
        x = pGrid[0];
        y = pGrid[1];
        if (bln3D) {
    ...
    public void grid2Geog() {
        double[] pGeog = bln3D ? Dataset_EVOL.grid2Geo(x, y, z) :
                         Dataset_EVOL.grid2Geo(x, y);
        lon = pGeog[1];
        lat = pGeog[0];
        if (bln3D) {
            depth = pGeog[2];
    ...
    public void increment(double[] move) {
        ...
        if (move.length > 2) {
            z += move[2];
            z = Math.max(0.d, Math.min(z, (double) Dataset_EVOL.get_nz() - 1.00001f));
        }
    }
...
//////////
    ...
    public void setXYD(double x, double y, double depth) {
        ...
        this.y = y;
        this.z = bln3D
                 ? Math.min(Dataset_EVOL.depth2z(x, y, depth), Dataset_EVOL.get_nz() - 1)
                 : 0;
    }
